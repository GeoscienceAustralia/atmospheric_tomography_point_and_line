Upwind.Downwind <- function(inv_data){
  ...
  for (i in 1:dim2){
    ...
    if (inv_data$release_rate[i] == 0){
      ...
      theta <- (inv_data$wind_dir[i] - 270) * pi / 180 # convert to radians
      ...
      if (is.na(inv_data$x2[i]) == TRUE){ # Towers
        if (inv_data$rot_x[i] < 0 || inv_data$rot_angle[i] < -angle || inv_data$rot_angle[i] > angle){
          inv_data$Position[i] <- "upwind"
        } else {
        ...
        if ((inv_data$rot_angle[i] >= -angle & inv_data$rot_angle[i] <= angle) || (inv_data$rot_las_angle[i] >= -angle & inv_data$rot_las_angle[i] <= angle)){
          inv_data$Position[i] <- "downwind"
        } else {
          inv_data$Position[i] <- "upwind"
        } # If either end point within +-angle degrees
      } else if ((inv_data$rot_angle[i] >= -angle & inv_data$rot_angle[i] <= angle) || (inv_data$rot_las_angle[i] >= -angle & inv_data$rot_las_angle[i] <= angle)){ 
        inv_data$Position[i] <- "downwind"
      } else { # If both end points outside +-angle degrees, check x-intercept
        x <- c(inv_data$x1[i], inv_data$x2[i])
        y <- c(inv_data$y1[i], inv_data$y2[i])
        model1 <- lm(y ~ x)
        xint <- - model1[[1]][1] / model1[[1]][2]
        if (xint < 0){
          inv_data$Position[i] <- "upwind"
        } else {
          inv_data$Position[i] <- "downwind"
